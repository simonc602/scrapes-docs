...
def render(
    ...
    try:
        ...
        print("Run: cd .claude/skills/viz-excalidraw-diagram/references && uv sync && uv run playwright install chromium", file=sys.stderr)
        sys.exit(1)
    ...
    with sync_playwright() as p:
        try:
            ...
            if "Executable doesn't exist" in str(e) or "browserType.launch" in str(e):
                ...
                print("Run: cd .claude/skills/viz-excalidraw-diagram/references && uv run playwright install chromium", file=sys.stderr)
                sys.exit(1)
            raise
        ...
        page.on("console", lambda msg: print(f"browser console: {msg.type}: {msg.text}", file=sys.stderr))
        page.on("pageerror", lambda err: print(f"browser page error: {err}", file=sys.stderr))
