...
For exact tokens, see [Visual style](/docs/contribute/visual-style).
...
* Name the exact role: Company Owner, Company Admin, Team Owner, Team Admin, or Team Member.
* Say when the actor is the local workspace or the hosted server.
...
* Explain that the hosted server does not trust identity sent by a local workspace.
...
## Distinguish permission terms

Team OS access and task permission modes are different controls.

* **Team OS Full access** controls resources inside one Team. It can come from
  the Company Owner role, a Company grant for a Company Admin, or a Team Owner
  or Team Admin role.
* **Full access** in a Goal disables Claude's task-level permission checks. It
  still cannot bypass Team OS grants or workspace and file boundaries.
* Team Members without Full access need explicit client grants and Team skill
  grants for hosted resources. Local installation skills remain available
  separately. State the required grant whenever you describe a protected
  action.
